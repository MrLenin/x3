...
#define KEY_LDAP_UUID "ldap_uuid"
...
#define KEY_LDAP_FIELD_UUID "ldap_field_uuid"
...
static const struct message_entry msgtab[] = {
    ...
    { "NSMSG_LDAP_IDENTITY_MISMATCH", "This account name belongs to a registration that no longer exists. Your credentials are valid, but they are not the ones this account was created with, so it cannot be opened. Please contact network staff." },
 ...
 */
#ifdef WITH_LDAP
/* Outcome of checking a handle's stored directory identity against the entry
 * that just authenticated. */
enum ldap_identity_result {
    LDAP_IDENT_OK,       /* the bound entry is the identity this handle belongs to */
    LDAP_IDENT_ADOPTED,  /* handle carried no identity yet; learned it just now */
    LDAP_IDENT_UNAVAIL,  /* directory cannot answer -- proceed unverified, warn */
    LDAP_IDENT_MISMATCH  /* the name was reused by a different identity -- refuse */
};

/* A handle name is a reusable label, not an identity. Deleting an account in
 * Keycloak/LDAP cascades the directory entry away but leaves this handle -- and
 * everything keyed to it, channel access and oper level included -- in saxdb.
 * Whoever registers the name next would otherwise bind straight onto those
 * privileges, because both auth paths join saxdb to the directory on nothing
 * but the name string.
 *
 * So compare the directory's immutable key instead. entryUUID survives a
 * modrdn (a real rename stays the same identity) and is regenerated by a
 * delete-and-recreate (a reused name is a different identity).
 *
 * Call only AFTER credentials have been accepted -- otherwise this becomes an
 * oracle for probing which names have been recycled.
 */
static enum ldap_identity_result
nickserv_verify_ldap_identity(struct handle_info *hi, const char *handle)
{
    char *uuid = NULL;
    int rc;

    if (!nickserv_conf.ldap_enable)
        return LDAP_IDENT_OK;
    if (!(nickserv_conf.ldap_field_uuid && *nickserv_conf.ldap_field_uuid))
        return LDAP_IDENT_OK; /* identity binding not configured */

    rc = ldap_get_user_uuid(handle, &uuid);
    if (rc != LDAP_SUCCESS || !uuid) {
        /* Fail OPEN, loudly. A directory that cannot produce the key is an
         * operator error; failing closed would lock every user out of the
         * network, which is strictly worse than the behaviour that existed
         * before binding. The warning is what stops this rotting unnoticed.
         */
        log_module(NS_LOG, LOG_WARNING, "LDAP identity binding: cannot read %s for %s (%s) -- "
                   "proceeding UNVERIFIED, name-reuse protection is NOT in effect.",
                   nickserv_conf.ldap_field_uuid, handle, ldap_err2string(rc));
        return LDAP_IDENT_UNAVAIL;
    }

    if (!hi->ldap_uuid) {
        /* Predates binding. Trust on first use: adopt whoever authenticates
         * now. This closes the vector going forward but cannot vouch for a
         * handle already stranded before the upgrade -- those need the
         * out-of-band census sweep, not this check.
         */
        hi->ldap_uuid = uuid; /* takes ownership */
        log_module(NS_LOG, LOG_INFO, "LDAP identity binding: account %s adopted directory identity %s.",
                   handle, uuid);
        return LDAP_IDENT_ADOPTED;
    }

    if (strcmp(hi->ldap_uuid, uuid)) {
        log_module(NS_LOG, LOG_ERROR, "LDAP identity binding: REFUSED auth for account %s -- handle is "
                   "bound to directory identity %s but the entry that authenticated is %s. The name has "
                   "been reused by a different identity; this handle's channel access and oper level are "
                   "NOT theirs.", handle, hi->ldap_uuid, uuid);
        free(uuid);
        return LDAP_IDENT_MISMATCH;
    }
...
#ifdef WITH_LDAP
    /* Runs on every path that reaches here, including SASL -- which arrives
     * with password == NULL because the ircd already proved the credential
     * against Keycloak and is only asking us to resolve the handle. That is
     * precisely the case this guards: the credential is valid for whoever
     * holds the name NOW, which is not necessarily who this handle belongs to.
     */
    if (nickserv_verify_ldap_identity(hi, hi->handle) == LDAP_IDENT_MISMATCH)
        return NULL;
#endif
...
#ifdef WITH_LDAP
    /* Unconditional: an sslfp auth skips the bind entirely, but the handle it
     * resolves is just as susceptible to having had its name reused. */
    if (nickserv_verify_ldap_identity(hi, hi->handle) == LDAP_IDENT_MISMATCH) {
        send_message_type(4, user, cmd->parent->bot,
                          handle_find_message(hi, "NSMSG_LDAP_IDENTITY_MISMATCH"));
        argv[pw_arg] = "BADIDENT";
        return 1;
    }
#endif
    if (HANDLE_FLAGGED(hi, SUSPENDED)) {
...
nickserv_saxdb_write(struct saxdb_context *ctx) {
    ...
    for (it = dict_first(nickserv_handle_dict); it; it = iter_next(it)) {
        ...
        if (hi->ldap_uuid)
            saxdb_write_string(ctx, KEY_LDAP_UUID, hi->ldap_uuid);
        if (hi->last_quit_host[0])
...
nickserv_db_read_handle(char *handle, dict_t obj)
{
    ...
    /* Absent for every handle written before identity binding existed; those
     * adopt on first successful auth (see nickserv_verify_ldap_identity). */
    str = database_get_data(obj, KEY_LDAP_UUID, RECDB_QSTRING);
    if (str)
        hi->ldap_uuid = strdup(str);
...
nickserv_conf_read(void)
{
    ...
    str = database_get_data(conf_node, KEY_LDAP_FIELD_UUID, RECDB_QSTRING);
    nickserv_conf.ldap_field_uuid = str ? str : "";
